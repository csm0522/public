	...
	<table class="table table-bordered">
		...
		<tbody>
			<?php if(is_array($list)): foreach($list as $k=>$C): ?><tr>
					...
					<td><?php if( $C['articaltype'] == 1) { echo '原创教程'; } else if( $C['articaltype'] == 2) { echo '观点'; } else if( $C['articaltype'] == 3) { echo '设计资讯';} else if( $C['articaltype'] == 4) { echo '其他';} ?>
					</td>
					...
					<td>
						<a href="/share/index.php/Admin/Works/editEssay/id/<?php echo ($C["ariticalid"]); ?>"><button class="btn btn-primary" type="button" style="background: lightskyblue;width: 82px;border:none">编辑</button></a>

					</td>
					<td>
						<?php if($C['status'] == 0 ): ?><label class="btn btn-primary" type="button" style="background: black;color:white;width: 82px;">已删除</label>
							<a href="/share/index.php/Admin/Works/unDeleEssay/id/<?php echo ($C["ariticalid"]); ?>" class="primary" ><button class="btn btn-primary" type="button"style="background: orange;color:white;width: 82px;">解除</button></a>
							<?php elseif($C['status'] == 1 ): ?> <a href="/share/index.php/Admin/Works/deleEssay/id/<?php echo ($C["ariticalid"]); ?>" class="primary" ><button class="btn btn-primary" type="button">删除文章</button></a><?php endif; ?>
					</td>
