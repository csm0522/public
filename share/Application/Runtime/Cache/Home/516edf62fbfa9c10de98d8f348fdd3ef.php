...
<div class="category">
 ...
 if($userInfo['username'] == '') { echo('昵称'); } else{ echo $userInfo[username]; } ?>
        ...
        <div class="info">
            <h style="font-size:14px;color:#666;padding-top:5%;"><?php echo ($userInfo['username']); ?>的个人主页</h>
        </div>
        <div class="more">
            <a>
                <span onclick="concern()"><p id="conres"></p></span></a>
        </div>
    </div>
...
<script>
    ...
    $(window).ready(function() {
        $.ajax({
            type: "get", //请求的方式
            url: "/share/index.php/Home/user/checkcon/id/" + j, //请求的url地址
    ...
    function concern(){
        ...
        if(res=="关注"){
            $.ajax({
                type: "get", //请求的方式
                url: "/share/index.php/Home/user/concern/id/" + j, //请求的url地址
                success: function (data) { //请求成功时，处理返回来的数据
                    var msg = data.msg;
                    $("#conres").html(msg);
                }
            ...
            $.ajax({
                type: "get", //请求的方式
                url: "/share/index.php/Home/user/delcon/id/" + j, //请求的url地址
                success: function (data) { //请求成功时，处理返回来的数据
                    var msg = data.msg;
                    $("#conres").html(msg);
                }
